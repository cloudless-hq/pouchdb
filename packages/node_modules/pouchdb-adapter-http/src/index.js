...
function HttpPouch(opts, callback) {
  ...
  api._changes = function (opts) {
    ...
    var fetchData = function (since, callback) {
      ...
      if (opts.continuous && opts.sse) {
        setup().then(function () {
          ...
          changes.addEventListener('heartbeat', hb => {
            console.log(hb, opts.heartbeat)
          })
    ...
    var fetched = function (err, res) {
      ...
      if ((opts.continuous && opts.sse && !(limit && leftToFetch <= 0)) || !finished) {
        // handle restart?
        // nextTick(function () { fetchData(lastFetchedSeq, fetched); });
      } else if ((opts.continuous && !(limit && leftToFetch <= 0)) || !finished) {
